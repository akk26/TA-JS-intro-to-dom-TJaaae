...
for (let all of allHrsArray) {
  all.style.backgroundColor = "antiquewhite"
}
...
border.forEach(elm => elm.style.border = "2px solid purple");

// Hide the box number 17 (last box).

let hiddenBox = document.querySelectorAll(`.box`)
hiddenBox[16].style.display = "none";

// Change the border of all the hr element from solid to dashed type

allHrsArray.forEach((elm) => (elm.style.border = "1px dashed tomato"));
...
let awesome = document.querySelectorAll(`.box`);

awesome[5].classList.add(`awesome-box`);

// Using the toggle classList property toggle the class `awesome-box` from box 2

let toggleBox = document.querySelectorAll(`.box`);

toggleBox[1].classList.toggle(`awesome-box`);

// Using the remove classList proeprty remove the class `awesome-box` from box 4

let rm4 = document.querySelectorAll(`.box`);
rm4[3].classList.remove(`awesome-box`);
...
let input = document.querySelector(".five");
...
document.querySelector(`.ip[type=text]`);

ip.value = "Enter your email!";
